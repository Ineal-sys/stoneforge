      ...
      <div className="flex-1 min-w-0">
        ...
        {hasThreadParent && (
          <div className="flex items-center gap-1 text-xs text-gray-500 dark:text-gray-400 mt-0.5" data-testid={`inbox-page-list-item-thread-${item.id}`}>
            ...
            <span>{t('inbox.replyTo')} {threadParentSenderName}</span>
          </div>
        )}
        ...
        {(threadReplies && threadReplies.length > 0) && (
          ...
          >
            <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 mb-3">
              <MessageSquare className="w-3 h-3" />
              <span className="font-medium">{t('inbox.repliesInThread', { count: threadReplies.length })}</span>
            </div>
            <div className="space-y-3">
              {threadReplies.map((reply) => {
                ...
                const formatReplyTime = (dateStr: string) => {
                  ...
                  if (minutes < 1) return t('time.justNow');
                  if (minutes < 60) return t('timeLib.minutesShort', { count: minutes });
                  if (hours < 24) return t('timeLib.hoursShort', { count: hours });
...
function InboxMessageEmptyState() {
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center text-gray-400 dark:text-gray-500" data-testid="inbox-page-content-empty">
      ...
      <p className="text-xs mt-3 text-gray-300 dark:text-gray-600">{t('inbox.navigateTip')}</p>
    </div>
  );
}
...
export function InboxPage() {
    ...
    if (event.elementType === 'inbox-item' && event.eventType === 'created') {
      ...
      if (recipientId === currentUser?.id) {
        ...
        const message = sourceType === 'mention'
          ? t('inbox.mentionedNotification')
          : t('inbox.directMessageNotification');
        toast.info(message, {
      ...
      <div className="h-full flex flex-col bg-[var(--color-bg)]" data-testid="inbox-page">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center text-gray-500 dark:text-gray-400">
            ...
            <p className="text-sm mt-1">{t('inbox.noUserDescription')}</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="h-full flex flex-col bg-[var(--color-bg)]" data-testid="inbox-page">
      {/* Header */}
      <PageHeader
        title={currentUser ? t('inbox.usersInbox', { name: currentUser.name }) : t('inbox.title')}
        icon={Inbox}
        iconColor="text-blue-500"
        subtitle={inboxCount?.count !== undefined && inboxCount.count > 0
          ? t('inbox.unread', { count: inboxCount.count })
      ...
      <div className={`flex items-center justify-between ${isMobile ? 'px-3' : 'px-6'} py-2 border-b border-[var(--color-border)] bg-gray-50 dark:bg-gray-900/50`}>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <option value="all">{t('inbox.filters.allSources')}</option>
            <option value="direct">{t('inbox.filters.directMessages')}</option>
            <option value="mention">{t('inbox.filters.mentions')}</option>
          </select>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <option value="newest">{t('inbox.sort.newestFirst')}</option>
            <option value="oldest">{t('inbox.sort.oldestFirst')}</option>
            <option value="sender">{t('inbox.sort.bySender')}</option>
          </select>
        </div>
      </div>
        ...
        >
          {inboxLoading ? (
            ...
            <div className="text-center py-8 px-4" data-testid="inbox-page-error">
              ...
              <p className="text-sm text-gray-700 dark:text-gray-300">{t('inbox.failedToLoad')}</p>
            ...
            <div className="text-center py-8 px-4" data-testid="inbox-page-filtered-empty">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">{t('inbox.noFilterMatch')}</p>
              <button
          ...
          {inboxData && inboxData.items.length > 0 && (
            <div className="text-center text-xs text-gray-500 dark:text-gray-400 py-2 border-t border-gray-100 dark:border-gray-800">
              {inboxSourceFilter !== 'all' ? (
                <>{t('inbox.showing', { filtered: filteredAndSortedInboxItems.length, total: inboxData.items.length })}</>
              ) : inboxData.hasMore ? (
              ...
              {showReplyComposer && selectedInboxItem.status !== 'archived' && (
                <div className="border-t border-gray-200 dark:border-gray-700 p-4 bg-gray-50 dark:bg-gray-800" data-testid="inbox-reply-composer">
                  <div className="flex items-center gap-2 mb-2 text-xs text-gray-500 dark:text-gray-400">
                    ...
                    <span>
                      {t('inbox.replyingAs')} <span className="font-medium text-gray-700 dark:text-gray-300">{currentUser?.name}</span> {t('inbox.to')} {selectedInboxItem.sender?.name ?? t('inbox.unknown')}
                    </span>
                  </div>
                  ...
                  {sendReplyMutation.isError && (
                    <p className="mt-2 text-xs text-red-500" data-testid="inbox-reply-error">
                      {sendReplyMutation.error instanceof Error
                        ...
                        : t('inbox.failedToSendReply')}
                    </p>
                  )}
                  <p className="mt-2 text-xs text-gray-400">
                    {t('inbox.sendShortcut', { key: navigator.platform.includes('Mac') ? 'Cmd' : 'Ctrl' })}
                  </p>
                </div>
              )}
            </>
