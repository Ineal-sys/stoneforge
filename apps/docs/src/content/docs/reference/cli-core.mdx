...
```bash
sf update <id> [--title <text>] [--status <s>] [--priority <1-5>] [--complexity <1-5>] [--description <text>] [--metadata <json>] [--tag <tag>] [--add-tag <tag>] [--remove-tag <tag>] [--assignee <id>]
```

Pass `--assignee ""` to unassign. Use `--tag` to replace all tags, or `--add-tag`/`--remove-tag` for incremental changes. Use `--description` to update a task's linked document or a document's content. Use `--metadata` to merge JSON into the element's metadata (null values remove keys).
